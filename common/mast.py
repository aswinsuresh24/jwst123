...
# Default HST science products used by hst123 / download notebooks
HST_PRODUCT_RULES = (
    ('c0m.fits', 'WFPC2'),
    ('c1m.fits', 'WFPC2'),
    ('c0m.fits', 'PC/WFC'),
...
DEFAULT_JWST_INSTRUMENTS = ('NIRCAM',)


def mast_login(token: Optional[str] = None) -> bool:
    """
    Authenticate to MAST with an authorization token.

    Mirrors hst123: ``Observations.login(token=...)`` so proprietary products
    available to the token holder can be queried and downloaded. See
    https://auth.mast.stsci.edu/info

    Parameters
    ----------
    token : str or None
        MAST API token. If None/empty, no login is attempted.
    ...
    try:
        ...
        print(f'WARNING: could not log in to MAST with input token: {exc}')
        return False
...
def query_hst(
    ...
    If ``token`` is provided, authenticate to MAST first and include proprietary
    observations (``public_only`` is forced False).
    """
    if token:
        mast_login(token)
        public_only = False
...
def query_jwst(
    ...
    If ``token`` is provided, authenticate to MAST first and include proprietary
    observations (``public_only`` is forced False). Without a token, only public
    data are returned by default.
    """
    if token:
        mast_login(token)
        public_only = False
...
def download_jwst_observations(
    ...
    Pass ``token`` to authenticate before downloading proprietary products.
    """
    if token:
        mast_login(token)
