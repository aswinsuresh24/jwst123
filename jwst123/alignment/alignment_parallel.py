...
def _bootstrap(repo: str) -> None:
    ...
    # Editable installs already expose jwst123; still allow repo-root on sys.path.
    ...
    for _attempt in range(2):
        try:
            with suppress_output():
                _sanitize_logging()
                import jwst123.alignment.relative_align  # noqa: F401
...
def _success_from_dict(data: dict[str, Any]):
    from jwst123.alignment.alignment_fallback import SuccessfulAlignment
...
def run_nircam_align_job(job: dict[str, Any]) -> AlignWorkerResult:
    ...
    from jwst123.alignment.alignment_wrap import (
        AlignmentSummaryRow,
    ...
    from jwst123.alignment.calibrators import max_reference_dispersion_mas
...
def _backup_alignment_products(outdir: Path, miri_path: str) -> list[tuple[Path, Path]]:
    ...
    from jwst123.alignment.alignment_wrap import find_jhat_product
...
def run_fallback_align_job(job: dict[str, Any]) -> AlignWorkerResult:
    ...
    from jwst123.alignment.alignment_wrap import harvest_alignment_metrics
