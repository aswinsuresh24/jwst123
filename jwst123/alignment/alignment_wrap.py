...
Example::
    ...
    Use ``python -m jwst123.scripts.alignment_wrap`` (CLI in scripts/).
...
def _resolve_repo_root(explicit: Path | None = None) -> Path:
    """Locate the installable jwst123 package / repo root for worker bootstrap."""
    if explicit is not None:
        root = explicit.expanduser().resolve()
        if not (root / 'jwst123' / 'alignment' / 'alignment_wrap.py').is_file() and not (
    ...
    here = Path(__file__).resolve().parent  # jwst123/
    candidates = [
        here.parent,  # repo root containing jwst123/
        Path.cwd(),
        Path('/data/rwisenbaker/jwst123'),
    ]
    for cand in candidates:
        if (cand / 'jwst123' / 'alignment' / 'alignment_wrap.py').is_file():
            return cand.resolve()
...
def _group_frames_by_filter(
    ...
    from jwst123.alignment.alignment_fallback import sort_frames_blue_to_red
...
def _run_jobs_parallel(
    ...
    *,
    ...
    from jwst123.alignment.alignment_parallel import AlignWorkerResult
    ...
    from jwst123.alignment.alignment_parallel import worker_initializer
...
def align_from_frames(
    ...
    *,
    ...
    from jwst123.alignment.calibrators import FILTER_MAX_REFERENCE_DISPERSION_MAS
    from jwst123.alignment.alignment_fallback import (
        SuccessfulAlignment,
        filter_wavelength_um,
        rank_fallback_parents,
    )
    from jwst123.alignment.alignment_parallel import run_fallback_align_job, run_nircam_align_job
    ...
    if use_filter_calibrators:
        from jwst123.alignment.calibrators import (
