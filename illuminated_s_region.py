...
class SRegionPolygon:
    ...
    def to_tangent_polygon(
        ...
        # Compact spherical offsets; cos(dec) on the center latitude.
    ...
    def to_pixel_polygon(
        ...
        *,
        ...
        ``max_roundtrip_arcsec``.  Plain ``world_to_pixel_values`` (and even
        ``all_world2pix(..., quiet=True)``) can return extrapolated detector
        coordinates for far off-axis sky positions; those must not be treated
        as valid footprint vertices (they caused false overlap detections).
        """
        ...
        # Angular residual accounting for cos(dec).
...
def main() -> None:
    ...
    s_region_string = s_region_polygon.to_string()
    print(s_region_string)
