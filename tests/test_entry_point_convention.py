"""Enforce that executable ``__main__`` blocks live under ``jwst123/scripts/``.
...
def test_no_main_blocks_outside_jwst123_scripts():
...
def test_required_cli_modules_exist():
    """Preserve alignment_wrap / jwst_download entry modules under jwst123/scripts/."""
    assert (REPO_ROOT / 'jwst123' / 'scripts' / 'alignment_wrap.py').is_file()
    assert (REPO_ROOT / 'jwst123' / 'scripts' / 'jwst_download.py').is_file()
    assert (REPO_ROOT / 'jwst123' / 'scripts' / 'download.py').is_file()
