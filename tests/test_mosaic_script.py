...
def test_mosaic_main_no_jhat_files(tmp_path: Path):
    ...
    with (
        patch('jwst123.scripts.mosaic.glob.glob', return_value=[]),
        patch('jwst123.scripts.mosaic.input_list', return_value=empty),
        patch('jwst123.scripts.mosaic.Pool'),
    ):
...
def test_apply_gwcs_main_with_paths(tmp_path: Path):
    ...
    with patch(
        'jwst123.scripts.apply_gwcs.apply_wcs_to_coadd',
